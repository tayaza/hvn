...
def sort_tissues(snp_genes):
    ...
    with open('../analysis/novel_snp_tissues.txt', 'wb') as wfile:
        writer = csv.writer(wfile, delimiter = '\t')
        writer.writerow(('Gene', 'SNP', 'SNP_Celltype', 'SNP_tissue'))
                ...
                if celltype in tissues[tissue]:
                    for gene in genes:
                        if gene in novel_genes:
                            to_file = gene, snp, celltype, tissue
                            writer.writerow(to_file)
    wfile.close()

if __name__== '__main__':
    t2d_snp_file = '../diabetes/results/dhs_results/gwas_snps.bed'
    obesity_eqtl_file = '../obesity/results/codes3d_results/dhs_results/gwas_snps.bed'
    t2d_summary_file = '../diabetes/results/summary.txt'
    obesity_summary_file = '../obesity/results/codes3d_results/summary.txt'
